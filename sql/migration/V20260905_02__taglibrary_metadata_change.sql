-- ----------------------------------------------------------------------------
-- V20260905_02 标签库维表关系 + 标签元数据变更迁移（向前迁移，可重复执行；不得包含 drop table）
-- 来源：ruoyi-taglibrary/src/main/resources/sql/taglibrary_metadata_migration.sql
-- 内容：
--   1. tl_tag_library_dimension 标签库默认码表关系表
--   2. tl_tag_metadata_change 标签元数据变更表
--   3. tl_audit_log 增加 request_id / detail_json 列，from_status/to_status 拓宽
--   4. 默认码表与批量映射权限按钮（menu_id 2130-2137）
-- 说明：菜单 2130-2135 插入已统一加 not exists 守卫，保证幂等
-- 注意：必须在 V20260905_03（批量映射同步升级，alter tl_tag_metadata_change）之前执行
-- ----------------------------------------------------------------------------
...
-- ----------------------------
insert into sys_menu select '2130', '默认码表查询', '2101', '9',  '#', '', '', '', 1, 0, 'F', '0', '0', 'taglibrary:library:dimension:list', '#', 'admin', sysdate(), '', null, '' from dual where not exists (select 1 from sys_menu where menu_id = '2130');
insert into sys_menu select '2131', '默认码表设置', '2101', '10', '#', '', '', '', 1, 0, 'F', '0', '0', 'taglibrary:library:dimension:set', '#', 'admin', sysdate(), '', null, '' from dual where not exists (select 1 from sys_menu where menu_id = '2131');
insert into sys_menu select '2132', '批量映射查询', '2102', '11', '#', '', '', '', 1, 0, 'F', '0', '0', 'taglibrary:tag:mapping:list', '#', 'admin', sysdate(), '', null, '' from dual where not exists (select 1 from sys_menu where menu_id = '2132');
insert into sys_menu select '2133', '映射草稿保存', '2102', '12', '#', '', '', '', 1, 0, 'F', '0', '0', 'taglibrary:tag:mapping:save', '#', 'admin', sysdate(), '', null, '' from dual where not exists (select 1 from sys_menu where menu_id = '2133');
insert into sys_menu select '2134', '映射提交审核', '2102', '13', '#', '', '', '', 1, 0, 'F', '0', '0', 'taglibrary:tag:mapping:submit', '#', 'admin', sysdate(), '', null, '' from dual where not exists (select 1 from sys_menu where menu_id = '2134');
insert into sys_menu select '2135', '元数据变更审批', '2103', '1',  '#', '', '', '', 1, 0, 'F', '0', '0', 'taglibrary:tag:mapping:audit', '#', 'admin', sysdate(), '', null, '' from dual where not exists (select 1 from sys_menu where menu_id = '2135');

-- 批量映射新增权限按钮（守卫可重复执行；存量库授权见 V20260905_03__tag_mapping_sync_upgrade.sql）
