...
| OceanBase 语义网络 §5/§9/§10 | 语义层独立`于物理层，Binding 用稳定 ID；图结构 + 字面索引 + 向量索引三种存储并存；大规模时"概览 → 业务域收敛 → 域内搜索"分级召回；Gold 集固定、按规模梯度加 hard negative 验证召回稳定性。` |
...
### 7.6 Python 侧模块结构（`tagpilot-semantic/tag_semantic/`）
...
| 服务间调用、Trace/反馈与状态回写   | `TsRuntimeClient.java`、`TsAgentClient.java`、`TsRetrievalService.java`、`TsSemanticController.java`                              |
| 隐私画像与可重建维护            | `TsProfileService.java`、`TsIndexMaintenanceService.java`                                                                       |
| Python 取用与不可变构建       | `tagpilot-semantic/tag_semantic/snapshot/{fetch,schema,canonicalize}.py`、`index/{builder,milvus_store,embedder,lease,maintenance}.py` |
| 检索与隔离评测               | `tagpilot-semantic/tag_semantic/server.py`、`retrieve/service.py`、`eval/acceptance.py`、`bootstrap/feedback_mining.py`                  |
| Agent 编排层                 | `tagpilot-agent/tagpilot_agent/{server,graph,dsl}.py`                                                                            |
| 两个治理页面                | `ruoyi-ui/src/views/taglibrary/semantic/`、`semantic-index/`                                                                    |
| 验证和运行                 | `bin/verify-tag-semantic.sh`、`bin/tag-semantic-runtime.sh`、`bin/tagpilot-agent.sh`、`tagpilot-semantic/scripts/`                       |
