...
> 范围：标签语义层（元数据、别名、口径、码值、版本）+ 检索索引（版本快照、词典、BM25、向量、索引构建与治理）
> 不含：查询理解 Prompt、受限选择、DSL 校验、LangGraph 编排（另文）
> 日期：2026-09-16　状态：**历史版本（v1，本地索引方案）**，仅供参考。现行版本见 [`标签语义层与检索索引建设方案.md`](./标签语义层与检索索引建设方案.md)（已改为 Milvus Standalone）。两版仅索引存储层（§1.3 决策、§3 索引层、§5.6 `ts_index_build`、§6.3、§7.2–7.6、S9、W1–W6、风险表）不同，语义层设计一致。
...
5. **建设流水线可执行且分阶段：** 码表入库与绑定 → 数据画像 → 规则分解建族 → LLM 批量富化草稿 → 人工复核 → 质量门禁 → 快照发布 → 索引构建 → Gold 集回归。MVP 不引入向量数据库。
...
- MVP 不上向量数据库：版本化快照 + BM25 + 内存 Embedding 矩阵。Embedding / Reranker 模型在同一 Benchmark 上 A/B 决定。
...
flowchart TB
    ...
    subgraph I["索引层（Python AI Runtime）"]
        DICT["别名词典 AC 自动机"]
        BM["BM25（jieba+自定义词典）"]
        EMB["Embedding 矩阵 (N×d)"]
        GRAPH["结构图：域→概念→族→标签→码值"]
    end
    ...
    C -- "规则解析" --> CV
    SEM --> CON
    SEM & AL & CV & CF & EX & BT & PF & T --> SNAP --> IB --> DICT & BM & EMB & GRAPH
...
### 6.3 索引产物布局（AI Runtime 侧）

```text
/data/tag-index/
  L107-20260916-001/                       # snapshot_id
    snapshot.jsonl                         # 原样拷贝，校验 content_hash
    graph.sqlite                           # 域/概念/族/标签/码值/易混淆 结构图（供 overview/subgraph 工具）
    bge-m3#tpl1/                           # build（模型 # 模板）
      alias_automaton.pkl                  # 精确/别名词典（AC 自动机），仅 reviewed 别名
      bm25.pkl                             # 三类文档（tag/concept/code_value）分别建索引
      emb_tag.npy  emb_concept.npy  emb_code.npy   # float32 矩阵
      doc_ids.json                         # 行号 → doc_id
      manifest.json                        # build_id、模型 hash、模板版本、产物 hash、eval_summary
```
...
| 通道 | 实现 | 输入 | 输出粒度 | 说明 |
|---|---|---|---|---|
| ① 精确/别名 | AC 自动机（`alias_automaton.pkl`），长词优先，单字需上下文 | 原子条件文本 | TAG/CONCEPT/CODE_VALUE | 只含 reviewed 别名；命中即高置信特征 |
| ② BM25 | `rank_bm25` / 自研 BM25F；jieba + 自定义词典（所有标签名、别名、码值标签、缩写） | 归一化查询（去否定词、去数字单位） | tag / concept / code 三索引各取 Top 30 | 名称字段权重 3，别名 2，其他 1 |
| ③ Dense | BGE-M3（候选，A/B：bge-large-zh-v1.5 / Qwen3-Embedding）；`query·E^T` 点积 | 原子条件文本 | 三矩阵各 Top 30 | 969×1024 矩阵内存 4 MB，无需 ANN |
...
```text
各通道排名 ──RRF(k=60)──▶ 候选 Top 50（tag 级；concept/code 命中展开为其 tag 或 tag+code）
  ──族聚合──▶ 按 family_key 归组，组内按 ④ 的时间锚点匹配度排序
  ──Reranker（bge-reranker-v2-m3，候选 50 → 打分）──▶ Top 10
  ──权限过滤（eligible tag set）──▶ 候选集（交给受限选择）
```
...
每个候选输出：`exact_hit(0/1)`, `alias_hit_type`, `bm25_rank`, `dense_rank`, `rrf_score`, `rerank_score`, `margin_to_2nd`, `channel_agreement(命中通道数)`, `family_resolved(0/1)`, `time_facet_conflict(0/1)`, `completeness_score`, `unresolved_fuzzy_terms`, `history_accept_rate`。这些字段随 Trace 落入 `ts_retrieval_feedback.rank_features`，用于训练/校准分流阈值。
...
3. 969 规模下全量检索；≥ 2000 时仍全量 BM25/Dense（成本可忽略），仅在 Reranker 前用域先验截断到 Top 80。
...
tag_semantic/
  ...
  docs/templates.py         # tpl1 文档模板（唯一实现，BM25/Embedding 共用）
  index/alias_index.py      # AC 自动机
  index/bm25_index.py
  index/dense_index.py      # 模型加载、批量编码、矩阵检索；记录模型 hash
  index/builder.py          # build(snapshot_id, model, tpl) → manifest.json
  retrieve/channels.py      # 五通道
  retrieve/fusion.py        # RRF、族聚合、reranker
  retrieve/facets.py        # 时间锚点/单位/否定/边界解析（与 ts_business_term 同源）
  retrieve/service.py       # retrieve(requirement, eligible_tag_ids, snapshot_id) → candidates + features
  eval/gold.py  eval/run.py # §10
  cli.py                    # build / eval / query
```

`IndexStore` 抽象接口预留 `LocalFileStore`（MVP）与 `OpenSearchStore`（生产）两种实现，检索服务不感知。
...
| **S9 索引构建与评测** | `cli build --snapshot L107-… --model bge-m3 --tpl tpl1` → manifest；`cli eval --gold gold_v1.jsonl` → Recall@10/20；达标则 `ACTIVE` | `ts_index_build` | AI Runtime |
...
| `POST /taglibrary/semantic/index-build` / `PUT …/{buildId}/status` | AI Runtime 回写构建登记与评测结果 |
...
| 周 | 内容 | 验收 |
|---|---|---|
| W1 | S0 码表入库绑定；S1 迁移与 Java 骨架；S2 规则初始化脚本 | `ts_tag_semantic` 969 条、码值语义 1723 条入库；族分组可查询 |
| W2 | S3 概念归并（黄金场景域优先）；S7 词典；快照导出与 JSONL Schema；Python loader + tpl1 + BM25/Dense 构建 | 首个 `L107-…-001` 快照 + build；基线 Recall@20 |
| W3 | S5 LLM 富化全量草稿；语义维护页；S6 复核黄金场景概念 | 黄金场景标签 completeness ≥ 90 |
| W4 | 五通道 + RRF + 族解析 + reranker A/B；`gold_retrieval_v1` 300 条；消融实验 | Recall@20 ≥ 99%（Gold 300） |
| W5 | S4 数据画像接入；`hard_negative_auto`、`scale_gradient`；反馈接口与回流任务；门禁自动化 | 门禁通过发布第 2 个快照；评测报告 |
| W6 | 全量复核收尾（均分 ≥ 85）；Gold 扩到 600；版本冻结 | 达 §2.1 目标；可回放任一 Trace |
