...
@Service
public class DpDatasetCatalogServiceImpl implements IDpDatasetCatalogService {
    ...
    @Override
    public List<DpDatasetCatalog> selectCatalogList(DpDatasetCatalog catalog) {
        return catalogMapper.selectCatalogList(catalog);
    }

    @Override
    public DpDatasetCatalog selectCatalogById(Long catalogId) {
     ...
     */
    @Override
    public int insertCatalog(DpDatasetCatalog catalog) {
        if (catalog.getParentId() == null || catalog.getParentId() == 0L) {
            catalog.setParentId(0L);
            catalog.setAncestors("0");
        } else {
            DpDatasetCatalog parent = catalogMapper.selectCatalogById(catalog.getParentId());
            if (parent == null) {
     ...
     */
    @Override
    public int updateCatalog(DpDatasetCatalog catalog) {
        DpDatasetCatalog newParent = catalog.getParentId() == null || catalog.getParentId() == 0L
            ? null : catalogMapper.selectCatalogById(catalog.getParentId());
        DpDatasetCatalog oldCatalog = catalogMapper.selectCatalogById(catalog.getCatalogId());
    ...
    private void updateCatalogChildren(Long catalogId, String newAncestors, String oldAncestors) {
        List<DpDatasetCatalog> children = catalogMapper.selectChildrenById(catalogId);
        for (DpDatasetCatalog child : children) {
    ...
    public int deleteCatalogById(Long catalogId) {
        ...
        if (datasetMapper.countByCatalogId(catalogId) > 0) {
            throw new ServiceException("该目录下存在数据集，不允许删除");
        }
    ...
    @Override
    @Transactional
    public int updateCatalogOrder(DpDatasetCatalog catalog) {
        DpDatasetCatalog old = catalogMapper.selectCatalogById(catalog.getCatalogId());
        if (old == null) {
        ...
        if (parentChanged) {
            ...
            if (newParentId == 0L) {
                ...
                DpDatasetCatalog newParent = catalogMapper.selectCatalogById(newParentId);
                if (newParent == null) {
