...
def build_agent(retrieve_service, selector: Selector | None = None):
    selector = selector or selector_from_env()

    def retrieve_node(state: AgentState):
        response = retrieve_service.retrieve(state["requirement"], state["eligible_tag_ids"], 20)
        candidates = visible_candidates(response["candidates"], retrieve_service.catalog, state["eligible_tag_ids"], 20)
        return {"retrieval": response, "candidates": candidates}

    def select_node(state: AgentState):
        if state["retrieval"].get("decision") in {"CLARIFY", "INEXPRESSIBLE"}:
            return {"proposal": None}
        return {"proposal": selector.select(state["requirement"], state["candidates"], retrieve_service.catalog)}
    ...
    def validate_node(state: AgentState):
        ...
        try:
            dsl = validate_dsl(proposal["dsl"], retrieve_service.catalog, state["eligible_tag_ids"])
