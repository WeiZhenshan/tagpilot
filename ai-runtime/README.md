...
Java 是快照、当前资格、激活状态和反馈审计的权威端。Python 消费不可变快照和 Java 传入的显式资格集合，在固定 build 内运行 BGE、Milvus、LangGraph Agent 和 DSL 校验。结果只是候选或待确认 DSL，`auto_execute=false`。
...
- LangGraph Agent 已接通；未配置生成式模型时页面必须显示 `model_connected=false`，只走精确证据演示模式。
...
两个进程必须使用同一个 `TAG_RUNTIME_TOKEN`。令牌只通过当前 shell 或本机受控配置注入，不写入 Git、命令文档或验收报告。

终端 1：启动真实 BGE/Milvus 运行时。
...
终端 2：启动 Java 和 Vue。本地演示允许使用从页面“导出实时冻结”得到的当前来源文件；必须同时固定其 SHA-256，两者缺一时配置不生效。
...
Agent 只把经过 Java 资格剪裁和检索后的候选发给 OpenAI-compatible `chat/completions` 端点，不发送客户原始数据或整库快照。只有组织批准的端点和数据范围才能配置：
...
```bash
# Milvus 实体行集与 manifest 对账（运行时已启动）
