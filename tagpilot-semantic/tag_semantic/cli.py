...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    built = load_index(Path(args.artifact))
    if content_hash(catalog.rows) != built['manifest']['content_hash']:
        raise ValueError('请求快照与构建不一致')
    ...
    eligible = set(args.eligible) if args.eligible is not None else None
...
def cmd_fetch(args):
    import os
    from tag_semantic.snapshot.fetch import fetch_snapshot
    fetch_snapshot(args.base_url, args.snapshot_id, os.environ["TAG_JAVA_TOKEN"], args.out)
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    fetch = sub.add_parser("fetch-snapshot")
    ...
    build.add_argument("--store-type", choices=["LOCAL", "MILVUS"], required=True)
    build.add_argument("--embedding-path")
    build.add_argument("--reranker-path")
