...
public class TsExpressionCompiler {
    ...
    private Result compile(JsonNode n, Long library, Set<Long> eligible, Map<Long,JsonNode> published, Map<String,JsonNode> caps,
        ...
        if(n==null || !n.isObject() || depth>8 || ++count[0]>64) fail("SCHEMA_INVALID","计算表达式格式或复杂度非法");
        for(String key:Arrays.asList("sql","field_name","table","function","column")) if(n.has(key)) fail("SCHEMA_INVALID","计算表达式不能指定物理字段或SQL");
        ...
        if("TAG".equals(kind)) {
            ...
            if(!eligible.contains(tid) || p==null || tag==null || !library.equals(tag.getLibraryId()) || !"数值型".equals(tag.getTagType())) fail("SCHEMA_INVALID","计算输入不是可用的数值标签");
            ...
            if(new BigDecimal(scale).signum()<=0) fail("UNIT_MISMATCH","指标单位倍率无效");
        ...
        if("CAPABILITY".equals(kind)) {
            ...
            if(cap==null || !"REVIEWED".equals(cap.path("review_status").asText()) || !cap.path("version").asText().equals(n.path("version").asText())) fail("UNSUPPORTED_CAPABILITY","计算能力尚未发布或版本已变化");
            if(!stack.add(id)) fail("UNSUPPORTED_CAPABILITY","计算能力依赖成环");
            for(JsonNode dep:cap.path("input_tag_ids")) if(!eligible.contains(dep.asLong())) fail("INELIGIBLE_TAG","计算能力依赖不在当前资格范围");
            checkCaliber(n,cap);
            ...
            else fail("SCHEMA_INVALID","业务定义不能直接用作数值指标");
            Set<Long> declared=new HashSet<>();for(JsonNode dep:cap.path("input_tag_ids")) declared.add(dep.asLong());
            if(!declared.containsAll(r.tagIds)) fail("UNSUPPORTED_CAPABILITY","计算能力的实际依赖超出发布清单");
            r.tagIds.addAll(declared);r.time=time(cap);r.grain=cap.path("grain").asText("CUSTOMER");
            if(!Objects.equals(r.unit,cap.path("unit").asText("NONE"))) fail("UNIT_MISMATCH","计算能力输出单位与发布定义不一致");
        ...
        if(!Arrays.asList("ADD","SUB","MUL","DIV","COUNT_POSITIVE").contains(kind) || !args.isArray()
                || ("COUNT_POSITIVE".equals(kind)?args.size()<1 || args.size()>12:args.size()!=2)) fail("SCHEMA_INVALID","计算算子或参数数量不受支持");
        ...
        for(JsonNode a:args) {
            ...
            if("COUNT_POSITIVE".equals(kind) && !unique.add(child.node.toString())) fail("SCHEMA_INVALID","类别计数包含重复输入");
        }
        if(grains.size()>1)fail("CALIBER_CONFLICT","计算输入粒度不同");
        if(times.size()>1) {
            if(!"EXPLICIT_PERIODS".equals(n.path("time_alignment").asText()))fail("CALIBER_CONFLICT","计算输入时间不同，需要明确跨期关系");
            for(int i=0;i<args.size();i++)if(!parts.get(i).tagIds.isEmpty() && args.get(i).path("expected_caliber").size()==0)fail("CALIBER_CONFLICT","跨期计算缺少逐项时间证据");
        }
        String a=parts.get(0).unit,b=parts.size()>1?parts.get(1).unit:a;
        if(Arrays.asList("ADD","SUB","COUNT_POSITIVE").contains(kind)) {
            for(Result p:parts)if(!Objects.equals(a,p.unit))fail("UNIT_MISMATCH","计算输入单位不同");
            r.unit="COUNT_POSITIVE".equals(kind)?"COUNT":a;
        } else if("DIV".equals(kind)) {
            if(a.equals(b))r.unit="RATIO";else if(dimensionless(b))r.unit=a;else fail("UNIT_MISMATCH","不支持这种单位的除法");
        } else {
            if(dimensionless(a))r.unit=b;else if(dimensionless(b))r.unit=a;else fail("UNIT_MISMATCH","乘法需要无量纲倍率");
        }
    ...
    private Result aggregate(JsonNode cap,Long library) {
        ...
        if(online==null || online.getVersionId()!=version) fail("VERSION_MISMATCH","聚合数据集版本不可用或已变化");
        ...
        if(rootSource==null || source==null || !Objects.equals(rootSource.getDatasourceId(),source.getDatasourceId())) fail("SCHEMA_INVALID","聚合模板必须与客户库使用同一数据源");
        String table;
        ...
        String agg=impl.path("aggregation").asText();if(!Arrays.asList("COUNT_DISTINCT","COUNT","SUM","MAX","MIN","AVG").contains(agg))fail("SCHEMA_INVALID","聚合方式未登记");
        if(agg.startsWith("COUNT") && !"COUNT".equals(r.unit))fail("UNIT_MISMATCH","计数聚合单位必须为COUNT");
        r.node.put("aggregation",agg);String empty=impl.path("empty_policy").asText("UNKNOWN");
        if(!Arrays.asList("UNKNOWN","ZERO_WHEN_NO_ROWS").contains(empty))fail("SCHEMA_INVALID","聚合空值规则非法");
        if(("ZERO_WHEN_NO_ROWS".equals(empty) || agg.startsWith("COUNT")) && !"FULL".equals(impl.path("coverage").asText()))fail("SCHEMA_INVALID","零事件计数需要发布完整覆盖证据");
        r.node.put("empty_policy",empty);ArrayNode filters=json.createArrayNode();
        if(impl.path("filters").size()>12)fail("SCHEMA_INVALID","聚合过滤项过多");
    ...
    public static void checkCaliber(JsonNode request,JsonNode published) {
        ...
        if(!request.path("time_constraint").asText().isEmpty() && (!expected.isObject()||expected.size()==0))fail("CALIBER_CONFLICT","时间要求缺少可核验的解释");
        if(!expected.isMissingNode() && !expected.isNull() && !expected.isObject())fail("CALIBER_CONFLICT","口径格式非法");
        expected.fields().forEachRemaining(e->{if(!e.getValue().isNull() && (!actual.hasNonNull(e.getKey()) || !caliberValue(e.getKey(),e.getValue(),actual).equals(caliberValue(e.getKey(),actual.get(e.getKey()),actual))))fail("CALIBER_CONFLICT","计算输入口径不匹配或发布信息缺失");});
    }
    private static void fail(String code,String message){throw new TsPlanValidationException(code,null,message);}
}
