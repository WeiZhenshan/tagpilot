...
public interface TsAgentThreadMapper {
    String COLUMNS="thread_id as threadId,user_id as userId,library_id as libraryId,title,archived,row_version as rowVersion,create_time as createTime,update_time as updateTime";
    @Select("select "+COLUMNS+" from ts_agent_thread where user_id=#{uid} and archived=#{archived} order by update_time desc limit #{limit} offset #{offset}")
    List<TsAgentThread> list(@Param("uid") Long uid, @Param("archived") String archived, @Param("limit") int limit, @Param("offset") int offset);
    ...
    @Insert("insert into ts_agent_thread(thread_id,user_id,library_id,title,archived,row_version,payload) values(#{threadId},#{userId},#{libraryId},#{title},'0',0,#{payload})")
    int insert(TsAgentThread row);
    @Update("update ts_agent_thread set title=#{title},archived=#{archived},payload=#{payload},row_version=row_version+1,update_time=now() where thread_id=#{threadId} and user_id=#{userId} and row_version=#{rowVersion}")
